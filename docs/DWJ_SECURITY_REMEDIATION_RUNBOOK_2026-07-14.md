...
- **`time_verses` → confirmed pure public reference content, branch taken: `security_invoker` recreation.** Its definition has no user/family-scoping column at all, and the table it reads from (`bible_verses`) already has an open `using (true)` `SELECT` policy — recreating with `security_invoker = true` reproduces that same, already-correct, intended-open visibility. `security_invoker` views require PostgreSQL 15+ — **confirmed 2026-07-15**: `select version()` returned PostgreSQL **17.6** (`server_version_num 170006`) on the live project, well above the minimum. The migration's fail-closed fallback note is now moot but left in the file as documentation of the reasoning.

**Dependency check on `family_table` (2026-07-15):** the `pg_depend`/`pg_rewrite` query from this section (which detects other views or rules built on top of a relation) returned **zero rows** — no other view or rule in the database is defined on top of `family_table`. This rules out the most common failure mode (a second view layered on this one). It does **not**, by itself, rule out a PL/pgSQL or SQL function that merely references `family_table` in its body text — Postgres does not record `pg_depend` entries for tables mentioned only inside a function body (function bodies are opaque strings at creation time), so a hidden function dependency wouldn't show up in this result. The function-body text-search query and the trigger query from this section were not run this pass. This is a lower-risk gap than it sounds: the chosen fix revokes grants only from `public`/`anon`/`authenticated`, not from the table owner, so a `SECURITY DEFINER` function running as owner (the normal pattern used throughout this package) would be unaffected either way. The residual risk is narrower — a `SECURITY INVOKER` function that expected the *calling* role to have a grant on `family_table` — which is an unusual, non-default pattern. Recommend running the function-body search before final production sign-off as a confidence step, not as a blocker.
...
**D.** Apply `20260714000001_security_primitives.sql` **only**. This adds the RPCs and `is_admin()` without restricting any current access. ~~Before this step, confirm the live Postgres version~~ — **done**: PostgreSQL 17.6, `security_invoker` views fully supported (Section 6).
...
- **Does anything outside this repository depend on `family_table` or `time_verses`?** Confirmed: nothing inside this repo does (no client call site, no Edge Function). The `pg_depend` dependency check (2026-07-15, Section 6) confirmed no other database view or rule is built on top of `family_table`. Not yet checked: whether any function body or trigger references it (see Section 6's note — narrower residual risk, not a blocker). Cannot rule out an external client, internal tool, or integration this session has no visibility into. If one exists and depends on direct `SELECT` of `family_table`, it will break once Section 8 is applied (by design — that access was never safe) and would need to be migrated to `get_my_family_table()`.
