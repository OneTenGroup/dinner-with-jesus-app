...
**Further update, same day:** Steve confirmed his admin UUID (`28356e7e-067c-49a8-81a2-095576c432a7`) directly against `auth.users` — returned `steve@onetengroup.ai`. He also ran metadata-inspection queries confirming `family_table` and `time_verses` are both `postgres`-owned views with no `security_invoker` option; `family_table` was a real, confirmed exposure (exposes every family's invite code and roster regardless of caller) and is now routed through a new `get_my_family_table()` RPC, while `time_verses` was confirmed as safe reference content and hardened defensively. All of this is now reflected in the runbook, which remains the single authoritative document.
